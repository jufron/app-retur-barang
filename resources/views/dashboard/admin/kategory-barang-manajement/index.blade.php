                                  ...
                                  <tbody>
                                    @foreach ($kategoryBarang as $kb)
                                      <tr>
                                          ...
                                          <td>{{ $kb->updated_at }}</td>
                                          <td>
                                            <form id="form-delete" action="{{ route('admin.category-barang.destroy', $kb) }}" method="post">
                                              @method('delete')
                                              @csrf
                                              <button id="button-info" class="btn btn-info rounded-4" type="button" data-url="{{ route('admin.category-barang.show', $kb) }}">
                                                <i class="fa-solid fa-info"></i>
                                              </button>
                                              <a href="{{ route('admin.category-barang.edit', $kb) }}" class="btn icon btn-warning text-white rounded-4">
